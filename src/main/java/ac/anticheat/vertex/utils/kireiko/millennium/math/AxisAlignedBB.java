package ac.anticheat.vertex.utils.kireiko.millennium.math;

import ac.anticheat.vertex.utils.kireiko.millennium.vectors.Vec3;
...
public final class AxisAlignedBB {
    ...
    public Vec3 clip(Vec3 start, Vec3 end) {
        ...
        if (maxX != null && (result == null || start.squareDistanceTo(maxX) < start.squareDistanceTo(result))) {
            result = maxX;
        }

        if (minY != null && (result == null || start.squareDistanceTo(minY) < start.squareDistanceTo(result))) {
            result = minY;
        }

        if (maxY != null && (result == null || start.squareDistanceTo(maxY) < start.squareDistanceTo(result))) {
            result = maxY;
        }

        if (minZ != null && (result == null || start.squareDistanceTo(minZ) < start.squareDistanceTo(result))) {
            result = minZ;
        }

        if (maxZ != null && (result == null || start.squareDistanceTo(maxZ) < start.squareDistanceTo(result))) {
            result = maxZ;
        }
